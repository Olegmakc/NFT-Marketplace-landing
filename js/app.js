(() => {
    ...
    function initSliders() {
        if (document.querySelector(".auctions__slider")) new core(".auctions__slider", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".auctions__controls .controls-slider__btn_prev",
                nextEl: ".auctions__controls .controls-slider__btn_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.1,
        ...
        if (document.querySelector(".colections__slider")) new core(".colections__slider", {
            modules: [ Navigation ],
            observer: true,
            observeParents: true,
            slidesPerView: 2.63,
            ...
            speed: 800,
            navigation: {
                prevEl: ".colections__controls .controls-slider__btn_prev",
                nextEl: ".colections__controls .controls-slider__btn_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.2,
        ...
        if (document.querySelector(".categories__slider")) new core(".categories__slider", {
            ...
            speed: 800,
            navigation: {
                prevEl: ".categories__controls .controls-slider__btn_prev",
                nextEl: ".categories__controls .controls-slider__btn_next"
            },
            breakpoints: {
                320: {
                    slidesPerView: 1.1,
    ...
    document.addEventListener("DOMContentLoaded", (function(e) {
        let auctionExpirationElements = document.querySelectorAll("[data-auction-end]");
        if (auctionExpirationElements.length > 0) for (let index = 0; index < auctionExpirationElements.length; index++) {
            const auctionExpirationElement = auctionExpirationElements[index];
            createExpirationTimer(auctionExpirationElement);
        }
        if (modules_flsModules.fullpage) {
            const header = document.querySelector("header.header");
            const headerScrollHeight = header.dataset.scroll ? +header.dataset.scroll : 1;
            const activeSection = modules_flsModules.fullpage.activeSection;
            let windowInnerHeight = window.innerHeight;
            if (activeSection.offsetHeight > windowInnerHeight || activeSection.scrollHeight > windowInnerHeight) activeSection.addEventListener("scroll", (e => {
                if (activeSection.scrollTop > headerScrollHeight) !header.classList.contains("_header-scroll") ? header.classList.add("_header-scroll") : null; else header.classList.contains("_header-scroll") ? header.classList.remove("_header-scroll") : null;
            }));
            document.addEventListener("fpswitching", (e => {
                let fp = e.detail.fp;
                if (0 !== fp.activeSectionId) !header.classList.contains("_header-scroll") ? header.classList.add("_header-scroll") : null; else if (fp.activeSection.scrollTop < headerScrollHeight) header.classList.contains("_header-scroll") ? header.classList.remove("_header-scroll") : null;
            }));
            let scrollDown = document.querySelector("button.hero__scroll-down");
            if (scrollDown) scrollDown.addEventListener("click", (function(e) {
                let gotoSectionId = scrollDown.dataset.gotoSection ? scrollDown.dataset.gotoSection : null;
                let gotoSection = document.querySelector(`${gotoSectionId}`) ? document.querySelector(`${gotoSectionId}`).closest("[data-fp-section]") : null;
                let searchSectionId = +gotoSection.dataset.fpId;
                modules_flsModules.fullpage.switchingSection(searchSectionId);
            }));
        }
        document.addEventListener("click", (() => {
            console.log(document.querySelector(".lot-hero").offsetTop);
        }));
    }));
    function createExpirationTimer(element) {
        const [hours, minutes, seconds] = element.dataset.auctionEnd ? String(element.dataset.auctionEnd).split(":") : null;
        let expirationDate = new Date;
        expirationDate.setHours(expirationDate.getHours() + +hours);
        expirationDate.setMinutes(expirationDate.getMinutes() + +minutes);
        expirationDate.setSeconds(expirationDate.getSeconds() + +seconds);
        const expirationDateId = setInterval((function() {
            let currentDate = (new Date).getTime();
            const auctionLeftTime = expirationDate - currentDate;
            const hours = Math.floor(auctionLeftTime % (1e3 * 60 * 60 * 24) / (1e3 * 60 * 60));
            const minutes = Math.floor(auctionLeftTime % (1e3 * 60 * 60) / (1e3 * 60));
            const seconds = Math.floor(auctionLeftTime % (1e3 * 60) / 1e3);
            element.innerHTML = `${paddZero(hours)}:${paddZero(minutes)}:${paddZero(seconds)}`;
            if (auctionLeftTime < 0) {
                clearInterval(expirationDateId);
